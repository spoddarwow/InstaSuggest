...
public class ImageUtils {
	 ...
	 * @param maxSideLength
	 *            the maximum side length of the scaled down instance. Has to be
	 *            > 0.
	 * @return the scaled image if the original image is bigger than the scaled
	 *         version, the original instance otherwise.
	 */
	public static BufferedImage scaleImage(BufferedImage image,
		...
		// create new image
		if (scaleFactor < 1
				&& (int) Math.round(originalWidth * scaleFactor) > 1
				&& (int) Math.round(originalHeight * scaleFactor) > 1) {
	...
	/**
	 * Scale image to an arbitrary shape not retaining proportions and aspect
	 * ratio.
	 *
	 * @param image
	 * @param width
	...
	public static BufferedImage cropImage(BufferedImage image, int fromX,
		...
		BufferedImage cropped = new BufferedImage(toX - fromX, toY - fromY,
				BufferedImage.TYPE_INT_RGB);
		// fast scale (Java 1.4 & 1.5)
		Graphics g = cropped.getGraphics();
		g.drawImage(image, 0, 0, cropped.getWidth(), cropped.getHeight(),
				fromX, fromY, toX, toY, null);
		return cropped;
	}

	/**
	 * Converts an image to grey. Use instead of color conversion op, which
	...
	/**
	 * Converts an image to a standard internal representation. Taken from
	 * OpenIMAJ. Thanks to these guys! http://sourceforge.net/p/openimaj
	 *
	 * @param bimg
	 * @return
	 */
	public static BufferedImage createWorkingCopy(BufferedImage bimg) {
		...
		if (bimg.getType() == BufferedImage.TYPE_INT_RGB) {
			...
			image = new BufferedImage(bimg.getWidth(), bimg.getHeight(),
					BufferedImage.TYPE_INT_RGB);
	...
	public static BufferedImage trimWhiteSpace(BufferedImage img) {
		...
		// As soon as a scan line encounters non-white (or non-black) pixels we
		// know there is actual image content.
		...
		white = true;
		while (white) {
			raster.getPixels(0, raster.getHeight() - 1 - trimBottom,
					raster.getWidth(), 1, pixels);
			for (int i = 0; i < raster.getWidth(); i++) {
				if (pixels[i] < thresholdWhite)
					white = false;
			}
		...
		white = true;
		while (white) {
			raster.getPixels(raster.getWidth() - 1 - trimRight, 0, 1,
		...
		BufferedImage result = new BufferedImage(raster.getWidth()
				- (trimLeft + trimRight), raster.getHeight()
				- (trimTop + trimBottom), BufferedImage.TYPE_INT_RGB);
		result.getGraphics().drawImage(img, 0, 0, result.getWidth(),
				result.getHeight(), trimLeft, trimTop,
				img.getWidth() - trimRight, img.getHeight() - trimBottom, null);
		return result;
	}
	...
	public static BufferedImage differenceOfGaussians(BufferedImage image) {
		...
		ConvolveOp gaussian1 = new ConvolveOp(new Kernel(5, 5,
				ImageUtils.makeGaussianKernel(5, 1.0f)));
		ConvolveOp gaussian2 = new ConvolveOp(new Kernel(5, 5,
				ImageUtils.makeGaussianKernel(5, 2.0f)));
	...
	/**
	 * Check if the image is fail safe for color based features that are
	 * actually using 8 bits per pixel RGB.
	 *
	 * @param bufferedImage
	 * @return
	 */
	public static BufferedImage get8BitRGBImage(BufferedImage bufferedImage) {
		...
		if (bufferedImage.getType() != ColorSpace.TYPE_RGB
